...
export function guideFor(kind:EnvKind){
  ...
  if(kind==='ios')return <div className="install-guide" role="status"><p>iPhone 收推送需先添加到主屏幕：</p><ol><li>用 <strong>Safari</strong> 打开本站</li><li>点底部中间的<strong>分享</strong>按钮（方框加箭头图标）</li><li>下滑点「<strong>添加到主屏幕</strong>」→「添加」</li><li>从桌面打开「知会」，在「个人账号」中开启推送</li></ol></div>;
  if(kind==='android')return <div className="install-guide" role="status"><p>推荐安装<strong>独立 App</strong>：不依赖浏览器、不依赖谷歌服务，班委发布通知后会直接在手机上提醒。</p><ol><li>点下方「<strong>下载安卓 App 安装包</strong>」，下载完成后打开安装</li><li>若提示「未知来源」，选择允许安装 / 仍要安装</li><li>打开「知会」App，登录一次（登录状态保留 30 天），首次启动允许通知权限</li></ol><p>不想装 App 也可以：在「个人账号」开启网页推送，需使用 Chrome 等支持推送的浏览器（国产浏览器、微信内不支持）。</p></div>;
