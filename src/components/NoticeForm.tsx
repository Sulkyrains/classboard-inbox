...
import { timeIssues } from '../shared/times';
export function NoticeForm({initial,onSave,onCancel,label='发布通知',hint}:{initial:NoticeInput;onSave:(n:NoticeInput)=>Promise<void>;onCancel:()=>void;label?:string;hint?:string}){
  const [n,set]=useState(initial),[busy,setBusy]=useState(false),[error,setError]=useState('');
  const issues=timeIssues(n);
  ...
  return <form className="notice-form" onSubmit={submit}>
    ...
    <div className="form-grid"><label>开始时间<input type="datetime-local" value={toInputDate(n.event_at)} onChange={e=>set({...n,event_at:fromInputDate(e.target.value)})}/></label><label>截止时间<input type="datetime-local" value={toInputDate(n.deadline_at)} onChange={e=>set({...n,deadline_at:fromInputDate(e.target.value)})}/></label></div>
    <p className="field-hint">时间均为北京时间。开始时间是活动或上课的起始时间，截止时间是提交、报名等最后期限；没有明确时间的通知可以留空。</p>
    {issues.length>0&&<div className="warning">{issues.map(w=><p key={w}>{w}</p>)}</div>}
