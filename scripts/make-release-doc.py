...
SITE_VERSION = '2.4.0'      # 网页版本轮
APP_VERSION = '2.3.2'       # 安卓安装包版本（这一轮没改 App，安装包不变）
VERSION_CODE = 12
UPDATED = '2026-09-24'
...
def build(out_dir):
    ...
    set_font(sub.add_run(f'发布说明 · {UPDATED} · 网页版 v{SITE_VERSION}'), size=10.5, color='6B7280')
    ...
    for text in [
        '班长多了一张「账号管理」表（只有班长能看到）：全班每个人是「已激活」还是「还没登录过」、最后登录时间、几台设备在线、'
        '有没有开通知、订阅没订阅日历、看过几条通知，一屏看明白，还能一键复制还没激活的名单去点名。',
        '替人重置密码：谁忘了密码，班长在「账号管理」里点「重置密码」，系统当场生成一个临时密码，只显示这一次，'
        '私聊发给本人即可；发出去之后这人在其他设备上的登录立刻失效，下次登录必须改成自己的密码，'
        '临时密码不会留在网页上、也不会写进任何记录。',
        '强制下线：手机丢了、或者账号借给别人用过，点「强制下线」就能让这个账号在所有设备上掉线，'
        '密码不用改，本人用原密码重新登录即可。',
        '账号上的每一步操作都留痕：重置密码、强制下线各记一条，写明谁在什么时候对哪个账号做了什么，同样只有班长能看。',
        '班长多了一把「删除」：通知管理里对没用的通知可以直接彻底删除，会弹窗二次确认；删除后通知与同学的已读记录一并清掉，'
        '找不回来（如果只是想不再展示，请用「归档」）。',
        '新增班委操作日志：班长在侧边栏能看到「操作日志」，谁、什么职位、什么时候，对哪条通知做了发布、编辑、审核、归档、'
        '撤销还是删除，逐条排好；通知被彻底删掉后，记录里的标题快照仍然保留。',
        '全账号同步更快：班委发布、修改、归档或删除通知后，同学的看板不用刷新，最多半分钟自动更新。',
    ...
    for text in [
        ...
        '彻底删除：只有班长能删（其他班委看不到删除键，误点了不会生效）；删除会连着同学的已读记录一起清除，无法恢复。',
        '操作日志：所有班委的发布、编辑、审核、归档、撤销、删除都会自动记一条日志，'
        '班长在侧边栏「操作日志」里可以查到谁在什么时候动了哪条通知；这张日志只有班长能看。',
        '账号管理：只有班长有「账号管理」这一页，能看谁还没激活、谁在用，帮同学重置密码（临时密码只显示一次，'
        '请私聊发给本人）或让丢失手机的同学强制下线；其他委员看不到这一页，也调不动相关接口。',
    ...
    for text in [
        ...
        '忘记密码？找班长在「账号管理」里重置，班长会拿到一个只显示一次的临时密码（请私聊发给你，别发群里），'
        '登录后请立刻改成自己的密码；如果手机丢了或账号借给别人用过，同样可以找班长让你强制下线。',
        '换了手机？重新用学号登录，并在新手机上再打开一次通知开关即可（旧设备会失效）。',
        '通知发错了？请班委在「通知管理」里归档（同学看不到、记录保留）；彻底删除只有班长能操作，删了就找不回来。',
        '消息发得晚或临时有变？以看板上的最新通知为准，旧通知过期后会自动置灰。',
    ]:
        bullet(doc, text)

    heading(doc, '七、版本与维护', 1)
    paragraph(doc, f'当前版本：网页版 {SITE_VERSION}（{UPDATED} 更新）；安卓 App {APP_VERSION}（versionCode {VERSION_CODE}）'
                   '——这一轮只改网页，已经装好的 App 不用重新下载，打开就是新版。'
