...
try{
  if(args.includes('--status')){
    const rows=run('SELECT student_id,display_name,role,must_change_password FROM users ORDER BY must_change_password DESC,student_id')[0].results;
    const idle=rows.filter(r=>r.must_change_password);
    for(const r of rows)console.log(`${r.must_change_password?'未激活':'已激活'}  ${r.student_id}  ${r.display_name}${r.role==='committee'?'（班委）':''}`);
    ...
    if(!args.includes('--roster')||!args.includes('--committee'))throw Error('需要 --roster JSON路径 和 --committee 姓名（或 --status 查看激活情况、--reissue 重发全部未激活账号、--reset 学号 重置单人）');
