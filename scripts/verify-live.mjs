...
const origin=process.argv[2]||'https://classboard-upc.pages.dev';
const get=path=>fetch(origin+path,{signal:AbortSignal.timeout(20000)});
const page=await get('/');assert.equal(page.status,200);assert.match(await page.text(),/知可而办/);assert.match(page.headers.get('Content-Security-Policy')||'',/frame-ancestors 'none'/);
