...
- 2026-09-24 删除功能核验：本地隔离库用无头 Chrome 跑三种职位 27 项（班长看得到删除键、取消不删、确认后卡片消失且刷新不回来、草稿也有删除键；学习委员与团支书都看不到删除键、班长发布的那条对委员显示锁定提示）；线上用临时账号与两条待审核通知复验：同学 / 委员 / 团支书删除均 403，班长版本不符 409、缺版本 400、删除 200 且通知、已读、审计记录一并消失、重复删除 404、缓存版本前进，两个域名的前端产物都含删除文案与危险色样式，跑完清理干净（库回到 29 人 / 1 条通知 / 订阅 0）。
...
- 集成验证覆盖真实 Worker 路由：登录、来源校验、游客权限、发布、筛选、置顶、乐观锁、待审内容不可见、审核、归档、撤销归档 / 撤销驳回、班长永久删除（含已读与审计记录级联清理）、精确去重、退出，以及班委职位显示与归档 / 编辑权限分级（委员越权返回 403）。
- 浏览器验收：发布通知 → 同学登录查看 → 分类筛选 → 导入 → 原文检查 → 审核发布；截图路径需用清晰中文截图单独验证。
- 发布前全量测试（2026-09-24）：`npm test` 90 条 → `npm run typecheck` → `npm run build` → `npm run test:integration` → 本地 DOM 走查 → 线上迁移核查 → 两个 Pages 项目部署同一份 dist → 线上端到端（临时账号 + SQL 直插通知，不触发推送，用完即清；覆盖日程去重、撤销归档 / 撤销驳回、iCal 事件名、班长永久删除与越权 403）→ 安装包链路（本地与线上 `classboard.apk` sha256 一致、`app-version.json` 与 `android-app/app/build.gradle` 同为 versionCode 12 / 2.3.2、两个域名均 no-cache）。
