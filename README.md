...
线上网站：[https://classboard-upc.pages.dev](https://classboard-upc.pages.dev/)。2026-09-13 已上线账号升级，连接 Cloudflare Pages 和远程 D1，提供 6 条示例通知。全站通知须登录并完成首次改密后才能查看。2026-09-24 上线班委职位与权限分级：7 位班委带职位，通知显示发布人，班长 / 团支书权限高于其他委员。同日发布前全量测试顺带补上两处入口：顶栏职位标签可直接打开个人账号，侧边栏「我们的班级」回到今日看板。同日再按班长反馈上线六项改进：日程一条通知只出现一次、「活动时间」更名「开始时间」并在卡片内同时标出截止时间、已完成的通知不再进日程、左侧日期显示「今天 / 明天 / 后天」、归档与驳回的通知可撤销、群消息导入对信息不完善的消息放宽识别。

交付验证：88 条单元测试、类型检查、构建和接口集成测试通过；本地浏览器（DOM 走查）确认首次改密门禁、个人设置入口、顶栏职位标签与侧边栏「我们的班级」两处新入口、班委与同学两种身份、日程去重与 `今天 / 明天 / 后天` 日期标签、撤销归档按钮与重新发布提示，以及 498px / 390px 窄屏无横向溢出；线上端到端用临时账号与直插通知复验发布人展示、委员越权 403、团支书归档后全班看板消失、撤销归档后重新出现并回到「已发布」、iCal 事件名带（开始）/（截止），跑完清理干净（库回到 29 人 / 1 条通知 / 订阅 0，撤销重推未触达任何设备）。可运行 `node scripts/verify-live.mjs` 复查线上匿名访问隔离。OCR 实际样例识别在上一版本验收，本次识别逻辑未更改。可选 LLM 未配置真实服务，默认使用规则解析；班委登录后从“群消息导入”进入草稿与审核流程。
