...
- 班委账号登录、发布、编辑、归档，乐观锁避免并发修改互相覆盖；通知卡片与详情显示发布人「职位 + 姓名」，班长 / 团支书权限高于其他委员；班长可彻底删除通知，并能在「操作日志」里回看班委对通知做过的每一次改动。
...
- 集成验证覆盖真实 Worker 路由：登录、来源校验、游客权限、发布、筛选、置顶、乐观锁、待审内容不可见、审核、归档、撤销归档 / 撤销驳回、班长永久删除（含已读级联清理）、班长专用操作日志（含删除留痕与越权 403）、精确去重、退出，以及班委职位显示与归档 / 编辑权限分级（委员越权返回 403）。
- 浏览器验收：发布通知 → 同学登录查看 → 分类筛选 → 导入 → 原文检查 → 审核发布；截图路径需用清晰中文截图单独验证。
- 发布前全量测试（2026-09-24）：`npm test` 95 条 → `npm run typecheck` → `npm run build` → `npm run test:integration` → 本地 DOM 走查（删除键 27 项、操作日志 23 项、轮询同步 26 项）→ 线上迁移核查（0009 应用后 `operation_log` 回填 2 条）→ 两个 Pages 项目部署同一份 dist → 线上端到端（临时账号 + SQL 直插通知，不触发推送，用完即清；覆盖日程去重、撤销归档 / 撤销驳回、iCal 事件名、班长永久删除与越权 403、操作日志越权 403 与删除留痕、全账号同步与 ETag）→ 安装包链路（本地与线上 `classboard.apk` sha256 一致、`app-version.json` 与 `android-app/app/build.gradle` 同为 versionCode 12 / 2.3.2、两个域名均 no-cache）。

## 目录

```text
src/App.tsx                 看板、日程、管理、操作日志
