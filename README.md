...
线上网站：[https://classboard-upc.pages.dev](https://classboard-upc.pages.dev/)。2026-09-13 已上线账号升级，连接 Cloudflare Pages 和远程 D1，提供 6 条示例通知。全站通知须登录并完成首次改密后才能查看。2026-09-24 上线班委职位与权限分级：7 位班委带职位，通知显示发布人，班长 / 团支书权限高于其他委员。同日发布前全量测试顺带补上两处入口：顶栏职位标签可直接打开个人账号，侧边栏「我们的班级」回到今日看板。同日再按班长反馈上线六项改进：日程一条通知只出现一次、「活动时间」更名「开始时间」并在卡片内同时标出截止时间、已完成的通知不再进日程、左侧日期显示「今天 / 明天 / 后天」、归档与驳回的通知可撤销、群消息导入对信息不完善的消息放宽识别。同日晚间收尾上线：侧边栏班级名改为「智科2602」；全部 29 个账号重置为统一初始密码并强制首次改密（密码不写入仓库，由班长口头传达）；iPhone 端补齐底部安全区留白、`100dvh` 高度和 16px 输入框（聚焦不再被放大）；静态资源加 `X-Robots-Tag: noindex, nofollow` 与 `robots.txt`，避免这个内部工具被搜索引擎收录；推送的 VAPID `sub` 换成 https 地址，以通过苹果 APNs 对 JWT 的校验。同日晚间按班长要求再加**永久删除**：通知管理页每张卡片上给班长一个红色「删除」键，二次确认后这条通知连同已读记录一起从库里消失（团支书与其他委员看不到这个键，他们用「归档」）。同日最后按班长要求补上**操作日志**：新建 `operation_log` 表（迁移 0009 建表并回填历史 `audit_log`），班委的发布、编辑、审核、归档、撤销、删除逐条记下操作人、职位、时间与字段明细，通知被彻底删除后标题快照仍在；这张日志只有班长能看（其他职位请求返回 403），班长侧边栏多一个「操作日志」页。同日收尾再按班长要求做**账号管理**页：迁移 0010 给 `users` 补 `last_login_at` 并新建 `account_log`，班长在侧边栏「账号管理」里能看到全班每个账号的激活状态与使用情况（最后登录、在线设备、推送订阅、日历订阅、已读条数），可以对任意账号一键生成一次性临时密码（旧会话立即失效、本人下次登录必须先改成自己的密码，密码只显示这一次）或强制下线（密码不变，只踢设备）；这一页同样只有班长可见，其他职位连接口都返回 403。

交付验证：100 条单元测试、类型检查、构建和接口集成测试通过；本地浏览器（DOM 走查）确认首次改密门禁、个人设置入口、顶栏职位标签与侧边栏两处新入口、班委与同学两种身份、日程去重与 `今天 / 明天 / 后天` 日期标签、撤销归档按钮与重新发布提示，以及 498px / 390px 窄屏无横向溢出；线上端到端用临时账号与直插通知复验发布人展示、委员越权 403、团支书归档后全班看板消失、撤销归档后重新出现并回到「已发布」、iCal 事件名带（开始）/（截止），跑完清理干净（库回到 29 人 / 1 条通知 / 订阅 0，撤销重推未触达任何设备）。本轮收尾另加：线上二次核对 29 个账号的密码散列全部可由统一初始密码推导（29/29）且都处于「需改初始密码」状态；线上实测「初始密码登录 → 看板接口 403 → 改密后放行、旧密码立即失效」；首页与 API 安全响应头齐备、`robots.txt` 生效；iPhone UA + 390×844 触屏模拟在本地隔离库和线上各跑一遍，线上 39 项覆盖登录 → 看板 → 个人账号 → 改密 → 独立模式全流程（无横向溢出、无 APK 入口、自检面板五行齐全、四步安装引导可见）；安卓 412×915 走查 20 项，安装包入口、App Links 校验、推送开关、日历订阅、改密与深色模式与 iPhone 逐项一致。班长「删除」键另跑了本地三职位 27 项 DOM 走查与线上端到端（团支书、学习委员越权 403；班长删除后通知与已读记录一并消失，重复删除 404）。操作日志与全账号同步又各跑一轮：本地隔离库 23 项（班长看得到入口、在界面上归档与删除后日志真的长出来且删除用红色标记、委员与团支书没有入口且接口 403）与 26 项（同学页面不刷新，委员发布 / 班长改标题 / 归档 / 撤销归档 / 删除都在 30 秒内自动跟上，实测 29.3—30.3 秒；四种身份列表始终一致）；线上端到端 10 组（日志越权 403、被拒的越权与冲突尝试不留日志、发布后五种身份可见、编辑与版本冲突、已读按账号隔离且同账号第二台设备同步、归档与撤销全账号跟随、删除留痕含标题快照与已读条数、重复删除 404、两域名产物一致），跑完库回到 29 人 / 1 条通知 / 0 条推送订阅 / 日志仅剩迁移回填的 2 条历史记录。账号管理单独又跑一轮：单元测试补 5 条（临时密码长度、字符集、不含易混字符、300 次采样不重复、自定义长度），集成测试新增越权 403 且不留记录、重置后旧密码与旧会话同时失效、临时密码登录后必须先改密、强制下线只踢设备不改密码、账号记录只记动作不记密码；本地隔离库 DOM 走查 66 项（班长有入口与统计、待激活 / 已激活 / 班委筛选与搜索、重置弹窗的一次性密码当场可登录而旧密码 401、强制下线后设备数归零且按钮置灰、记录含操作人职位与注销会话数且不含密码明文；学习委员 / 团支书 / 同学都没有入口且接口 403），操作日志原有 23 项走查同步回归通过。可运行 `node scripts/verify-live.mjs` 复查线上匿名访问隔离。真机横幅推送（APNs / FCM）仍需用户在自己手机上确认。OCR 实际样例识别在上一版本验收，本次识别逻辑未更改。可选 LLM 未配置真实服务，默认使用规则解析；班委登录后从“群消息导入”进入草稿与审核流程。
...
- 班委账号登录、发布、编辑、归档，乐观锁避免并发修改互相覆盖；通知卡片与详情显示发布人「职位 + 姓名」，班长 / 团支书权限高于其他委员；班长可彻底删除通知，并能在「操作日志」里回看班委对通知做过的每一次改动；班长还能在「账号管理」里查看全班每个人的激活状态与使用情况，重置密码、让设备下线。
...
- 权限规则集中在 `src/shared/roles.ts`：**班长、团支书不受限制**，可以归档、编辑（含置顶）任何通知；**其他委员不能归档、也不能编辑班长或团支书发布的通知**，其余情况（自己发布的通知、委员之间）不受限。前端把受限的按钮置灰并说明原因，服务端同样返回 403。已归档或已驳回的通知可以「撤销」：撤销归档会重新发布并重新推送，撤销驳回回到待审核。**永久删除（`canDeleteNotice`）只给班长**：`DELETE /api/admin/notices/:id` 校验职位与 `version`，把通知连同 `notice_reads`、老 `audit_log` 的历史行一起删掉，并写一条删除日志；团支书与其他委员调用返回 403（理由指明只有班长可以删除）。
- 任何班委发布通知都直接公开、无需审核；「待审核 → 发布 / 驳回」只用于群消息导入生成的草稿。
- 归档即通知从所有人（含同学）的看板消失，只在班委「通知管理 → 已归档」中可查；没有面向同学的个人归档页。归档可在管理页「撤销归档」恢复为已发布并重新推送一次。
- **删除是班长的最后手段**：它不保留通知本身（归档才保留），确认弹窗里写明「无法恢复」并提示改用归档；删掉的瞬间 `notice_feed` 版本 +1，所有客户端下次拉取就会看到它消失。删掉的通知不会从操作日志里消失（见下）。
- **操作日志（`operation_log`，迁移 0009）**：班委的导入待审核、发布、审核通过、驳回、撤销驳回、编辑、归档、撤销归档、删除都记一条，含操作人姓名与职位、时间、以及编辑时改动过的字段名；`notice_title` 是当时的标题快照，所以通知被彻底删除后日志仍然可读。表上没有外键也是为此——`audit_log.notice_id` 有外键，通知一删历史就跟着没了，老表因此保留但不再写入。写入用 `INSERT … SELECT … FROM notices WHERE <与本批 UPDATE/DELETE 相同的条件>`（`logOp`），没落库的请求就不会多出日志，越权 403、版本冲突 409、参数 400 都不留痕。查询接口 `GET /api/admin/operations` 在服务端校验职位，只有班长能看，其他职位返回 403（前端藏入口只是顺带）。
- **账号管理（仅班长，迁移 0010）**：`GET /api/admin/accounts` 返回全班账号的激活状态（`must_change_password`）与使用情况（`last_login_at`、在线设备数、推送订阅数、是否订阅日历、已读条数），`salt / digest` 绝不进接口；`POST /api/admin/accounts/:id/reset-password` 生成 10 位随机临时密码（字母表去掉 `0O1lI`，用拒绝采样保证每个字符等概率，生成逻辑在 `src/shared/accounts.ts`，导入工具用的是同一套字母表），写库时只存散列、把账号标成「需首次改密」并注销其全部会话，明文只在这条 `no-store` 响应里出现一次，不写日志、不落库；`POST /api/admin/accounts/:id/logout-all` 只注销会话、不改密码（手机丢了或账号借给别人用完之后）。两者都校验职位（非班长 403「只有班长可以管理账号」），重置另有限速（同一班长 15 分钟 30 次）。`last_login_at` 只在登录成功的那一批写操作里更新，失败或被限速的登录不会污染它。记录落在 `account_log`（与 `operation_log` 分开：那张表是通知维度的，前端也渲染在通知日志页），同样是快照标签、无外键，只记动作、被操作的账号与注销会话数，不记密码。
...
- 2026-09-24 删除功能核验：本地隔离库用无头 Chrome 跑三种职位 27 项（班长看得到删除键、取消不删、确认后卡片消失且刷新不回来、草稿也有删除键；学习委员与团支书都看不到删除键、班长发布的那条对委员显示锁定提示）；线上用临时账号与两条待审核通知复验：同学 / 委员 / 团支书删除均 403，班长版本不符 409、缺版本 400、删除 200 且通知与已读记录一并消失、重复删除 404、缓存版本前进，两个域名的前端产物都含删除文案与危险色样式，跑完清理干净（库回到 29 人 / 1 条通知 / 订阅 0）。
- 2026-09-24 账号管理核验：本地隔离库 DOM 走查 66 项——班长一侧（入口、统计「已激活 4 / 5、待激活 1、班委 3、在线设备 4」、班委排在同学前、待激活 / 已激活 / 班委筛选与搜索、未激活名单复制提示、重置密码弹窗给出的一次性密码只显示一次且旧密码当场 401、新密码登录 200 并被要求改密、重置后该账号标成待激活但保留最后登录、强制下线后设备数归零且按钮置灰、账号操作记录两条含操作人职位与注销会话数、记录里没有密码明文），其他身份一侧（学习委员 / 团支书 / 同学的侧边栏都没有入口，`GET /api/admin/accounts` 与重置 / 下线接口全部 403 且理由明确，三者的通知管理与首页不受影响），全程无横向溢出；集成测试新增同一链路的接口级断言（越权尝试不留记录、重置后旧会话 401、临时密码走完首次改密后业务接口放行、强制下线不改密码）。
- 2026-09-24 线上账号管理端到端：用 4 个临时账号（班长 / 团支书 / 学习委员 / 同学）复验——班长的列表含 29 个真实账号与 4 个临时账号且响应里没有散列字段；登录写入最后登录时间；重置密码给出一次性临时密码，旧密码与旧会话同时失效，用临时密码登录后必须先改密；强制下线只注销会话、原密码仍可登录；团支书 / 学习委员 / 同学的列表、重置、下线请求全部 403（班委收到「只有班长可以管理账号」，同学止步于「仅班委可进行此操作」）且不留记录；账号记录两条含操作人职位与注销会话数、不含密码；两个域名前端产物均含账号管理文案。跑完清理，库回到 29 人 / 2 会话 / 0 通知 / 0 条推送订阅 / 操作日志 3 条 / 账号记录 0 条。
- 2026-09-24 操作日志与全账号同步核验：本地隔离库 DOM 走查 23 项（班长有「操作日志」入口、初始为空态、在界面上归档与删除后日志真的长出来、删除用红色标记并保留标题快照、委员与团支书没有入口且 `GET /api/admin/operations` 返回 403）与 26 项轮询同步（同学页面全程不刷新，委员发布 → 同学端 29.3 秒出现、班长改标题 30.3 秒更新、归档 30.3 秒移除、撤销归档 29.3 秒重现、删除 30.3 秒消失；四类身份列表逐轮一致，已读标记不跨账号）。线上用 5 个临时账号 + 3 条待审核通知跑 10 组：越权与版本冲突尝试全部被挡且日志总数不变；发布后五种身份都可见（ETag 304 → 200 → 304）；两次成功编辑各留一条日志、冲突那次不留；已读 / 已完成按账号隔离且同账号第二台设备同步；归档与撤销全账号跟随；删除后标题快照与「含 1 条已读记录一并清除」都在、重复删除 404；两个域名产物均含操作日志入口。跑完清理，日志只剩迁移回填的 2 条历史记录。
...
- 集成验证覆盖真实 Worker 路由：登录、来源校验、游客权限、发布、筛选、置顶、乐观锁、待审内容不可见、审核、归档、撤销归档 / 撤销驳回、班长永久删除（含已读级联清理）、班长专用操作日志（含删除留痕与越权 403）、班长账号管理（越权 403、一次性临时密码重置与旧密码 / 旧会话失效、强制下线不改密码、记录不含密码）、精确去重、退出，以及班委职位显示与归档 / 编辑权限分级（委员越权返回 403）。
- 浏览器验收：发布通知 → 同学登录查看 → 分类筛选 → 导入 → 原文检查 → 审核发布；截图路径需用清晰中文截图单独验证。
- 发布前全量测试（2026-09-24，账号管理这一轮）：`npm test` 100 条 → `npm run typecheck` → `npm run build` → `npm run test:integration` → 本地 DOM 走查（账号管理 66 项、操作日志 23 项、删除键 27 项、轮询同步 26 项）→ 线上迁移核查（0009 应用后 `operation_log` 回填 2 条；0010 应用后 `users.last_login_at` 就位、`account_log` 为空表）→ 两个 Pages 项目部署同一份 dist → 线上端到端（临时账号 + SQL 直插通知，不触发推送，用完即清；覆盖日程去重、撤销归档 / 撤销驳回、iCal 事件名、班长永久删除与越权 403、操作日志越权 403 与删除留痕、账号管理越权 403 / 重置一次性密码 / 强制下线、全账号同步与 ETag）→ 安装包链路（本地与线上 `classboard.apk` sha256 一致、`app-version.json` 与 `android-app/app/build.gradle` 同为 versionCode 12 / 2.3.2、两个域名均 no-cache）。

## 目录

```text
src/App.tsx                 看板、日程、管理、操作日志、账号管理
src/components/ImportPanel.tsx  文本 / OCR / 草稿交互
src/components/AccountAdmin.tsx 班长账号管理页（激活状态、重置密码、强制下线）
src/shared/accounts.ts      账号类型与一次性临时密码生成
src/shared/ops.ts           编辑字段差异（日志详情用）与日志类型
...
- 部署前需配置密钥：
  ...
  4. 首次部署需应用数据库迁移：`npx wrangler d1 migrations apply classboard-inbox --remote`（新增 0004 推送订阅表、0005 日历令牌列、0008 班委职位列并初始化 7 位班委、0009 操作日志表）。
