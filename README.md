...
线上网站：[https://classboard-upc.pages.dev](https://classboard-upc.pages.dev/)。2026-09-13 已上线账号升级，连接 Cloudflare Pages 和远程 D1，提供 6 条示例通知。全站通知须登录并完成首次改密后才能查看。2026-09-24 上线班委职位与权限分级：7 位班委带职位，通知显示发布人，班长 / 团支书权限高于其他委员。同日发布前全量测试顺带补上两处入口：顶栏职位标签可直接打开个人账号，侧边栏「我们的班级」回到今日看板。同日再按班长反馈上线六项改进：日程一条通知只出现一次、「活动时间」更名「开始时间」并在卡片内同时标出截止时间、已完成的通知不再进日程、左侧日期显示「今天 / 明天 / 后天」、归档与驳回的通知可撤销、群消息导入对信息不完善的消息放宽识别。同日晚间收尾上线：侧边栏班级名改为「智科2602」；全部 29 个账号重置为统一初始密码并强制首次改密（密码不写入仓库，由班长口头传达）；iPhone 端补齐底部安全区留白、`100dvh` 高度和 16px 输入框（聚焦不再被放大）；静态资源加 `X-Robots-Tag: noindex, nofollow` 与 `robots.txt`，避免这个内部工具被搜索引擎收录；推送的 VAPID `sub` 换成 https 地址，以通过苹果 APNs 对 JWT 的校验。

交付验证：88 条单元测试、类型检查、构建和接口集成测试通过；本地浏览器（DOM 走查）确认首次改密门禁、个人设置入口、顶栏职位标签与侧边栏两处新入口、班委与同学两种身份、日程去重与 `今天 / 明天 / 后天` 日期标签、撤销归档按钮与重新发布提示，以及 498px / 390px 窄屏无横向溢出；线上端到端用临时账号与直插通知复验发布人展示、委员越权 403、团支书归档后全班看板消失、撤销归档后重新出现并回到「已发布」、iCal 事件名带（开始）/（截止），跑完清理干净（库回到 29 人 / 1 条通知 / 订阅 0，撤销重推未触达任何设备）。本轮收尾另加：线上二次核对 29 个账号的密码散列全部可由统一初始密码推导（29/29）且都处于「需改初始密码」状态；线上实测「初始密码登录 → 看板接口 403 → 改密后放行、旧密码立即失效」；首页与 API 安全响应头齐备、`robots.txt` 生效；iPhone UA + 390×844 触屏模拟在本地隔离库和线上各跑一遍，线上 39 项覆盖登录 → 看板 → 个人账号 → 改密 → 独立模式全流程（无横向溢出、无 APK 入口、自检面板五行齐全、四步安装引导可见）；安卓 412×915 走查 20 项，安装包入口、App Links 校验、推送开关、日历订阅、改密与深色模式与 iPhone 逐项一致。可运行 `node scripts/verify-live.mjs` 复查线上匿名访问隔离。真机横幅推送（APNs / FCM）仍需用户在自己手机上确认。OCR 实际样例识别在上一版本验收，本次识别逻辑未更改。可选 LLM 未配置真实服务，默认使用规则解析；班委登录后从“群消息导入”进入草稿与审核流程。
...
- 标题、正文、发布时间、开始时间、截止时间、地点、通知对象、四种分类、置顶、过期和个人已读同步。
- 班委账号登录、发布、编辑、归档，乐观锁避免并发修改互相覆盖；通知卡片与详情显示发布人「职位 + 姓名」，班长 / 团支书权限高于其他委员。
- QQ / 微信文本导入，批量草稿、字段修正、原文对照、重复内容跳过；没认出来的片段会列出来，可一键转成草稿人工补齐。
...
- 权限规则集中在 `src/shared/roles.ts`：**班长、团支书不受限制**，可以归档、编辑（含置顶）任何通知；**其他委员不能归档、也不能编辑班长或团支书发布的通知**，其余情况（自己发布的通知、委员之间）不受限。前端把受限的按钮置灰并说明原因，服务端同样返回 403。已归档或已驳回的通知可以「撤销」：撤销归档会重新发布并重新推送，撤销驳回回到待审核。
- 任何班委发布通知都直接公开、无需审核；「待审核 → 发布 / 驳回」只用于群消息导入生成的草稿。
- 归档即通知从所有人（含同学）的看板消失，只在班委「通知管理 → 已归档」中可查；没有面向同学的个人归档页。归档可在管理页「撤销归档」恢复为已发布并重新推送一次。
...
4. 点击「生成通知草稿」，检查标题、分类、地点、开始与截止时间。
5. 对不确定字段进行修改，选中需要导入的条目，加入待审核。
6. 进入「通知管理 → 待审核」，对照原文后审核发布，或驳回。

所有时间按 **Asia/Shanghai** 解释，数据库保存带时区的 ISO 时间；没有明确钟点时不自动补午夜。“已过期”优先按截止时间判断，无截止时按开始时间判断；长期通知不过期。日程里一条通知只出现一次：优先按开始时间，开始时间已过而截止时间未到时改用截止时间；已完成的通知不进入日程，卡片内同时标出开始与截止。
...
返回 `drafts`、`engine`、`warnings`、`ignored`、`skipped`（未识别片段及原因，可在导入页转成草稿）。适配器只负责返回候选结构，不具备发布能力。消息最多 20,000 字，每批最多 30 个草稿；精确重复正文会在待审和已发布范围内跳过。不同措辞的近似重复、复杂跨消息指代、撤回与旧通知更新仍需人工判断。
...
- 解析单测覆盖典型群消息、相对时间、半点、跨年、无效日期、多草稿、变更提醒及 LLM 异常 / 超时 / 坏结构回退；新增月份相对表达（月底 / 月初 / 三天内 / 一周内 / 半个月内）、中文数字钟点与日期、时间段取结束时间、`周日晚` 组合、时间早于基准与跨年告警，以及未识别片段进 `skipped` 清单。
- 集成验证覆盖真实 Worker 路由：登录、来源校验、游客权限、发布、筛选、置顶、乐观锁、待审内容不可见、审核、归档、撤销归档 / 撤销驳回、精确去重、退出，以及班委职位显示与归档 / 编辑权限分级（委员越权返回 403）。
- 浏览器验收：发布通知 → 同学登录查看 → 分类筛选 → 导入 → 原文检查 → 审核发布；截图路径需用清晰中文截图单独验证。
- 发布前全量测试（2026-09-24）：`npm test` 88 条 → `npm run typecheck` → `npm run build` → `npm run test:integration` → 本地 DOM 走查 → 线上迁移核查 → 两个 Pages 项目部署同一份 dist → 线上端到端（临时账号 + SQL 直插通知，不触发推送，用完即清；覆盖日程去重、撤销归档 / 撤销驳回、iCal 事件名）→ 安装包链路（本地与线上 `classboard.apk` sha256 一致、`app-version.json` 与 `android-app/app/build.gradle` 同为 versionCode 12 / 2.3.2、两个域名均 no-cache）。
...
scripts/make-release-doc.py 生成发布说明 Word（python-docx，改版本号后重跑）
```
...
点击通知卡片空白处、标题或内容均可打开详情；键盘聚焦卡片后可按 Enter / 空格打开。标记已读和班委操作按钮不会误触详情。顶栏右上角职位标签与侧边栏「智科2602」都可点击：前者打开个人账号设置，后者回到今日看板并重置搜索与筛选。
...
- 部署前需配置密钥：
  ...
  3. 线上：`npx wrangler pages secret put VAPID_PUBLIC_KEY`、`npx wrangler pages secret put VAPID_PRIVATE_KEY`，再可配置 `VAPID_SUBJECT`（可省略；填写时请用 `https://` 网站地址或 `mailto:` 邮箱，苹果 APNs 会校验 JWT 的 `sub`，非法值会退回 403 BadJwtToken）。
